 ...
 *
 *   Copyright 2007-2024 Solomon Peachy (pizza@shaftnet.org)
 *
...
{
  DEFINE_PAPER_SIMPLE( "w288h432", "4x6", PT(1382,334), PT(2048,334), DYESUB_LANDSCAPE),
  DEFINE_PAPER( "w288h432-div2", "2x6*2", PT1(1382,334), PT(2048,334), 0, PT1(0,334), 0, 0, DYESUB_LANDSCAPE),
  DEFINE_PAPER_SIMPLE( "B7", "3.5x5", PT(1210,334), PT(1728,334), DYESUB_LANDSCAPE),
  DEFINE_PAPER_SIMPLE( "w360h504", "5x7", PT(1728,334), PT(2380,334), DYESUB_PORTRAIT),
...
static const dyesub_printsize_t sony_d711_printsize[] =
{
  ...
  { "300x300", "w215h215", 896, 896},
  { "300x300", "w215h287", 896, 1196},
  { "300x300", "w215h645", 896, 2688},
...
static void sony_upd711_printer_init_func(stp_vars_t *v)
{
  ...
  stp_zfwrite("\x83\xff\xff\xff\xfc\xff\xff\xff"
              "\xfb\xff\xff\xff\xf5\xff\xff\xff", 1, 16, v);

  stp_put32_le(20, v);
  stp_zfwrite("\x1b\x15\x00\x00\x00\x0d\x00"
              "\x00\x00\x00\x00\x01\x01\x43", 1, 14, v);
  stp_put16_be(0, v);
  stp_put16_be(pd->w_size, v);
  stp_put16_be(pd->h_size, v);
...
static void sony_upd711_printer_end_func(stp_vars_t *v)
{
  ...
  stp_put32_be(0xecffffff, v);
  stp_put32_be(pd->privdata.sonymd.printinfo, v);
  ...
  stp_put32_le(12, v);
  stp_zfwrite("\x1b\xc0\x00\x01\x00\x05\x00\x02", 1, 8, v);
  stp_zfwrite("\x0a\x00\x01", 1, 3, v);
  stp_putc(pd->privdata.sonymd.papertype, v);

  stp_put32_le(12, v);
  stp_zfwrite("\x1b\xc0\x00\x01\x00\x05\x00\x02", 1, 8, v);
  stp_zfwrite("\x0c\x00\x01", 1, 3, v);
  stp_putc(pd->privdata.sonymd.longfeed, v);

  stp_put32_le(12, v);
  stp_zfwrite("\x1b\xc0\x00\x01\x00\x05\x00\x02", 1, 8, v);
  stp_zfwrite("\x0b\x00\x01", 1, 3, v);
  stp_putc(pd->privdata.sonymd.printinfo, v);

  stp_put32_le(18, v);
  stp_zfwrite("\x1b\xe1\x00\x00\x00\x0b\x00\x00\x08\00", 1, 10, v);
  stp_put16_be(0x00, v); // XXX GG GG, based on print size?  Blank lines?
  dyesub_nputc(v, 0x0, 2);
  stp_put16_be(pd->w_size, v);
  stp_put16_be(pd->h_size, v);
			    ...
			    stp_parameter_t *description)
{
  ...
  else if (strcmp(name, "LongFeedAfter") == 0)
    {
      description->deflt.integer = 1;
      description->bounds.integer.lower = 0;
      description->bounds.integer.upper = 255;
      description->is_active = 1;
    }
  else if (strcmp(name, "PrintInfo") == 0)
    {
      description->is_active = 1;
      description->deflt.boolean = 0;
    }
  else if (strcmp(name, "SonyMediaType") == 0)
    {
      description->bounds.str = stp_string_list_create();
...
static int sony_upd711_parse_parameters(stp_vars_t *v)
{
  dyesub_privdata_t *pd = get_privdata(v);
  const char *gamma = stp_get_string_parameter(v, "SonyGamma");
  const char *media = stp_get_string_parameter(v, "SonyMediaType");
  ...
  pd->privdata.sonymd.advance = stp_get_int_parameter(v, "Advance");
  pd->privdata.sonymd.sharp = stp_get_int_parameter(v, "Sharpen");
  pd->privdata.sonymd.longfeed = stp_get_int_parameter(v, "LongFeedAfter");
  pd->privdata.sonymd.printinfo = stp_get_boolean_parameter(v, "PrintInfo");
  ...
  if (!strcmp(media, "HG")) {
    pd->privdata.sonymd.papertype = 1;
  } else {
    pd->privdata.sonymd.papertype = 0;
  }
...
static const dyesub_pagesize_t kodak_605_page[] =
{
  ...
  DEFINE_PAPER_SIMPLE( "w432h432-div2", "3x6*2", PT(1844,300), PT(1882,300), DYESUB_PORTRAIT),
  DEFINE_PAPER_SIMPLE( "w432h576", "6x8", PT(1844,300), PT(2434,300), DYESUB_PORTRAIT), /* 6x8 */
  DEFINE_PAPER_SIMPLE( "w432h576-div2", "4x6*2", PT(1844,300), PT(2490,300), DYESUB_PORTRAIT), /* 6x8-div2 */
};
...
static const dyesub_printsize_t kodak_605_printsize[] =
{
  ...
  { "300x300", "w288h432-div2", 1282, 1844},
  { "300x300", "w432h432", 1836, 1844},
  { "300x300", "w432h432-div2", 1844, 1836},
...
static const dyesub_pagesize_t kodak_9810_page[] =
{
  ...
  DEFINE_PAPER( "c8x10-div2", "8x5*2", PT(2464,300), PT1(3024,300), 0, 0, 0, PT1(0,300), DYESUB_PORTRAIT),
  DEFINE_PAPER_SIMPLE( "w576h864", "8x12", PT(2464,300), PT(3624,300), DYESUB_PORTRAIT),
  DEFINE_PAPER( "w576h864-div2", "8x6*2", PT(2464,300), PT1(3624,300), 0, 0, 0, PT1(0,300), DYESUB_PORTRAIT),
};
...
static const dyesub_pagesize_t kodak_8810_page[] =
{
  ...
  DEFINE_PAPER( "w576h576-div2", "8x4*2", PT1(2408,300), PT(2464,300), 0, PT1(0,300), 0, 0, DYESUB_LANDSCAPE),
  DEFINE_PAPER_SIMPLE( "c8x10", "8x10", PT(2464,300), PT(3024,300), DYESUB_PORTRAIT),
  DEFINE_PAPER( "c8x10-div2", "8x5*2", PT(2464,300), PT1(3024,300), 0, 0, 0, PT1(0,300), DYESUB_PORTRAIT),
  DEFINE_PAPER_SIMPLE( "A4", "203x297mm", PT(2464,300), PT(3531,300), DYESUB_PORTRAIT),
  DEFINE_PAPER_SIMPLE( "w576h864", "8x12", PT(2464,300), PT(3624,300), DYESUB_PORTRAIT),
  DEFINE_PAPER( "w576h864-div2", "8x6*2", PT(2464,300), PT1(3624,300), 0, 0, 0, PT1(0,300), DYESUB_PORTRAIT),
  DEFINE_PAPER( "w576h864-div3", "8x4*3", PT(2464,300), PT2(3624,300), 0, 0, 0, PT2(0,300), DYESUB_PORTRAIT),
  /* Panorama sizes */
...
static const dyesub_pagesize_t mitsu_cp9550_page[] =
{
  DEFINE_PAPER_SIMPLE( "B7", "3.5x5", PT(1240,346), PT(1812,346), DYESUB_LANDSCAPE),
  DEFINE_PAPER_SIMPLE( "w288h432", "4x6", PT(1416,346), PT(2152,346), DYESUB_LANDSCAPE),
  DEFINE_PAPER( "w288h432-div2", "2x6*2", PT1(1416,346), PT(2152,346), 0, PT1(0,346), 0, 0, DYESUB_LANDSCAPE),
  DEFINE_PAPER_SIMPLE( "w360h504", "5x7", PT(1812,346), PT(2452,346), DYESUB_PORTRAIT),
  DEFINE_PAPER_SIMPLE( "w432h576", "6x8", PT(2152,346), PT(2792,346), DYESUB_PORTRAIT),
...
static const dyesub_pagesize_t mitsu_cpd70x_page[] =
{
  ...
  DEFINE_PAPER( "w288h432-div2", "2x6*2", PT1(1228,300), PT(1864,300), 0, PT1(0,300), 0, 0, DYESUB_LANDSCAPE),
  DEFINE_PAPER_SIMPLE( "w360h504", "5x7", PT(1568,300), PT(2128,300), DYESUB_PORTRAIT),
  DEFINE_PAPER( "w360h504-div2", "3.5x5*2", PT(1568,300), PT1(2128,300), 0, 0, 0, PT1(0,300), DYESUB_PORTRAIT),
  ...
  DEFINE_PAPER( "w432h576-div2", "4x6*2", PT(1864,300), PT1(2730,300), 0, 0, PT(236,300), PT1(0,300), DYESUB_PORTRAIT),
#if 0 /* Theoretically supported, no way to test */
  DEFINE_PAPER( "w432h576-div4", "2x6*4", PT(1864,300), PT2(2730,300), 0, 0, PT(51,300), PT2(51,300), DYESUB_PORTRAIT),
  DEFINE_PAPER( "w432h648-div3", "3x6*3", PT(1864,300), PT3(2730,300), 0, 0, PT(3, 300), PT3(3, 300), DYESUB_PORTRAIT),
  DEFINE_PAPER( "w432h648-div2", "4.4x6*2", PT(1864,300), PT4(2730,300), 0, 0, PT(1, 300), PT4(1, 300), DYESUB_PORTRAIT),
#endif
};
...
static const dyesub_pagesize_t mitsu_cpk60_page[] =
{
  ...
  DEFINE_PAPER( "w288h432-div2", "2x6*2", PT1(1218,300), PT(1864,300), 0, PT1(0,300), 0, 0, DYESUB_LANDSCAPE),
  DEFINE_PAPER_SIMPLE( "w360h504", "5x7", PT(1568,300), PT(2128,300), DYESUB_PORTRAIT),
...
static const dyesub_pagesize_t mitsu_cpd90_page[] =
{
  ...
  DEFINE_PAPER( "w288h432-div2", "2x6*2", PT1(1226,300), PT(1852,300), 0, PT1(0,300), 0, 0, DYESUB_LANDSCAPE),
...
static const dyesub_printsize_t mitsu_cpd90_printsize[] =
{
  ...
  { "300x300", "w432h648", 1852, 2729}, // XXX is this right?
  { "300x300", "w432h648-div2", 1852, 2728}, // XXX ???
...
static const dyesub_pagesize_t mitsu_cpm1_page[] =
{
  ...
  DEFINE_PAPER( "w288h432-div2", "2x6*2", PT1(1232,300), PT(1852,300), 0, PT1(0,300), 0, 0, DYESUB_LANDSCAPE),
  ...
  DEFINE_PAPER( "w432h576-div2", "4x6*2", PT(1852,300), PT1(2434,300), 0, 0, 0, PT1(0, 300), DYESUB_PORTRAIT),
...
static const dyesub_pagesize_t shinko_chcs2145_page[] =
{
  ...
  DEFINE_PAPER_SIMPLE( "w288h432", "4x6", PT(1240,300), PT(1844,300), DYESUB_LANDSCAPE),
  DEFINE_PAPER( "w288h432-div2", "2x6*2", PT1(1240,300), PT(1844,300), 0, PT1(0,300), 0, 0, DYESUB_LANDSCAPE),
  DEFINE_PAPER_SIMPLE( "B7", "3.5x5", PT(1088,300), PT(1548,300), DYESUB_LANDSCAPE),
  DEFINE_PAPER_SIMPLE( "w360h504", "5x7", PT(1548,300), PT(2140,300), DYESUB_PORTRAIT),
...
static const dyesub_pagesize_t shinko_chcs1245_page[] =
{
  ...
  DEFINE_PAPER( "c8x10-div2", "8x5*2", PT(2446,300), PT1(3070,300), 0, 0, 0, PT1(0,300), DYESUB_PORTRAIT),
...
static const dyesub_pagesize_t shinko_chcs6145_page[] =
{
  ...
  DEFINE_PAPER_SIMPLE( "w432h576",	"6x8", PT(1844,300), PT(2434,300), DYESUB_PORTRAIT),
  DEFINE_PAPER( "w432h576-w432h432_w432h144", "6x6+2x6", PT(1844,300), PT1(2434,300), 0, 0, 0, PT1(0,300), DYESUB_PORTRAIT),
  DEFINE_PAPER_SIMPLE( "w432h576-div2", "4x6*2", PT(1844,300), PT(2492,300), DYESUB_PORTRAIT),
  DEFINE_PAPER_SIMPLE( "w432h648", "6x9", PT(1844,300), PT(2740,300), DYESUB_PORTRAIT),
};
...
static const dyesub_pagesize_t shinko_chcs2245_page[] =
{
  ...
  DEFINE_PAPER_SIMPLE( "w432h576",	"6x8", PT(1844,300), PT(2434,300), DYESUB_PORTRAIT),
  DEFINE_PAPER( "w432h576-div4",	"2x6*4", PT(1844,300), PT1(2434,300), 0, 0, 0, PT1(0,300), DYESUB_PORTRAIT),
  DEFINE_PAPER_SIMPLE( "w432h576-div2", "4x6*2", PT(1844,300), PT(2492,300), DYESUB_PORTRAIT),
};
...
static const dyesub_pagesize_t dnpds40_page[] =
{
  ...
  DEFINE_PAPER( "w288h432-div2", "2x6*2", PT1(1240,300), PT(1920,300), 0, PT1(0,300), PT(38,300), PT(38,300), DYESUB_LANDSCAPE),
  ...
  DEFINE_PAPER( "w432h576-div4", "2x6*4", PT(1920,300), PT1(2436,300), PT(38,300), PT(38,300), 0, PT1(0,300), DYESUB_PORTRAIT),
...
static const dyesub_pagesize_t dnpds80_page[] =
{
  ...
  DEFINE_PAPER( "c8x10-w576h432_w576h288", "8x6+8x4", PT(2560,300), PT1(3102,300), PT(56,300), PT(56,300), 0, PT1(0,300), DYESUB_PORTRAIT),
  ...
  DEFINE_PAPER( "w576h864-w576h576_w576h288", "8x8+8x4", PT(2560,300), PT1(3702,300), PT(56,300), PT(56,300), 0, PT1(0,300), DYESUB_PORTRAIT),
...
static const dyesub_pagesize_t dnpds80dx_page[] =
{
  ...
  DEFINE_PAPER( "w576h864-w576h576_w576h288", "8x8+8x4", PT(2560,300), PT1(3702,300), PT(56,300), PT(56,300), 0, PT1(0,300), DYESUB_PORTRAIT),
  DEFINE_PAPER( "w576h864-div3", "8x4*3", PT(2560,300), PT(3768,300), PT(56,300), PT(56,300), 0, 0, DYESUB_PORTRAIT),
  DEFINE_PAPER( "w576h864-div3sheet", "8x4*3 SHEET", PT(2560,300), PT2(3702,300), PT(56,300), PT(56,300), 0, PT2(0,300), DYESUB_PORTRAIT),
};
...
static const dyesub_pagesize_t dnpds620_page[] =
{
  ...
  DEFINE_PAPER( "w243h432", "3.375x6", PT4(1240,300), PT(1920,300), PT4(194,300), 0, PT(38,300), PT(38,300), DYESUB_LANDSCAPE),
  DEFINE_PAPER( "w252h432", "3.5x6", PT2(1240,300), PT(1920,300), PT2(96,300), 0, PT(38,300), PT(38,300), DYESUB_LANDSCAPE),
  DEFINE_PAPER( "w270h432", "3.75x6", PT3(1240,300), PT(1920,300), PT3(78,300), 0, PT(38,300), PT(38,300), DYESUB_LANDSCAPE),
  DEFINE_PAPER( "w288h432", "4x6", PT(1240,300), PT(1920,300), 0, 0, PT(38,300), PT(38,300), DYESUB_LANDSCAPE),
  DEFINE_PAPER( "w288h432-div2", "2x6*2", PT1(1240,300), PT(1920,300), 0, PT1(0,300), PT(38,300), PT(38,300), DYESUB_LANDSCAPE),
  ...
  DEFINE_PAPER( "w432h432",	"6x6", PT(1836,300), PT(1920,300), 0, 0, PT(38,300), PT(38,300), DYESUB_LANDSCAPE),
  DEFINE_PAPER( "w432h576", "6x8", PT(1920,300), PT(2436,300), PT(38,300), PT(38,300), 0, 0, DYESUB_PORTRAIT),
  DEFINE_PAPER( "w432h576-w432h432_w432h144", "6x6+2x6", PT(1920,300), PT1(2436,300), PT(38,300), PT(38,300), 0, PT1(0,300), DYESUB_PORTRAIT),
  DEFINE_PAPER( "w432h576-div4", "2x6*4", PT(1920,300), PT2(2436,300), PT(38,300), PT(38,300), 0, PT2(0,300), DYESUB_PORTRAIT),
  DEFINE_PAPER( "w432h576-div2", "4x6*2", PT(1920,300), PT(2498,300), PT(38,300), PT(38,300), 0, 0, DYESUB_PORTRAIT),
  DEFINE_PAPER( "w432h648", "6x9", PT(1920,300), PT(2740,300), PT(38,300), PT(38,300), 0, 0, DYESUB_PORTRAIT),
...
static const dyesub_pagesize_t dnp_qw410_page[] =
{
  ...
  DEFINE_PAPER( "w288h288-div2", "2x4*2", PT1(1236,300), PT(1408,300), 0, PT1(0,300), PT(71,300), PT(71,300), DYESUB_LANDSCAPE),
  DEFINE_PAPER( "w288h216", "4x3", PT(1836,300), PT(1408,300), PT(900,300), 0, PT(71,300), PT(71,300), DYESUB_LANDSCAPE),
  DEFINE_PAPER( "w288h288_w288h144", "4x4+4x2", PT(1408,300), PT1(1836,300), PT(71,300), PT(71,300), 0, PT1(0,300), DYESUB_PORTRAIT),
  DEFINE_PAPER( "w288h432", "4x6", PT(1408,300), PT(1836,300), PT(71,300), PT(71,300), 0, 0, DYESUB_PORTRAIT),
  DEFINE_PAPER( "w288h432-div2", "4x3*2", PT(1408,300), PT2(1836,300), PT(71,300), PT(71,300), 0, PT2(0,300), DYESUB_PORTRAIT),
  DEFINE_PAPER( "w288h432-div3", "2x4*3", PT(1408,300), PT3(1836,300), PT(71,300), PT(71,300), 0, PT3(0,300), DYESUB_PORTRAIT),
  DEFINE_PAPER( "w288h576", "4x8", PT(1408,300), PT(2436,300), PT(71,300), PT(71,300), 0, 0, DYESUB_PORTRAIT),
  DEFINE_PAPER( "w288h432_w288h144", "4x6+4x2", PT(1408,300), PT1(2436,300), PT(71,300), PT(71,300), 0, PT1(0,300), DYESUB_PORTRAIT),
  DEFINE_PAPER( "w288h432-div2_w288h144", "4x3*2+4x2", PT(1408,300), PT2(2436,300), PT(71,300), PT(71,300), 0, PT2(0,300), DYESUB_PORTRAIT),
  DEFINE_PAPER( "w288h576-div2", "4x4*2", PT(1408,300), PT3(2436,300), PT(71,300), PT(71,300), 0, PT3(0,300), DYESUB_PORTRAIT),
  DEFINE_PAPER( "w288h576-div4", "2x4*4", PT(1408,300), PT4(2436,300), PT(71,300), PT(71,300), 0, PT4(0,300), DYESUB_PORTRAIT),
  ...
  DEFINE_PAPER( "w324h432-div2", "4.5x3*2", PT(1408,300), PT1(1836,300), 0, 0, 0, PT1(0,300), DYESUB_PORTRAIT),
  DEFINE_PAPER( "w324h486", "4.5x6.75", PT(1408,300), PT(2436,300), 0, 0, PT(375,300), 0, DYESUB_PORTRAIT),
  // XXX 4.5x4.5 + 4.5x2 / 4.5x3 / 4.5x3.5 ?
  DEFINE_PAPER( "w324h432-div3", "4.5x2*3", PT(1408,300), PT2(1836,300), 0, 0, 0, PT2(0,300), DYESUB_PORTRAIT),
  DEFINE_PAPER( "w324h576", "4.5x8", PT(1408,300), PT(2436,300), 0, 0, 0, 0, DYESUB_PORTRAIT),
  DEFINE_PAPER( "w324h576-div2", "4.5x4*2", PT(1408,300), PT1(2436,300), 0, 0, 0, PT1(0,300), DYESUB_PORTRAIT),
  DEFINE_PAPER( "w324h576-div4", "4.5x2*4", PT(1408,300), PT2(2436,300), 0, 0, 0, PT2(0,300), DYESUB_PORTRAIT),
  DEFINE_PAPER( "w324h432_w324h144", "4.5x6+4.5x2", PT(1408,300), PT3(2436,300), 0, 0, 0, PT3(0,300), DYESUB_PORTRAIT),
  DEFINE_PAPER( "w324h432-div2_w324h144", "4.5x3*2+4.5x2", PT(1408,300), PT4(2436,300), 0, 0, 0, PT4(0,300), DYESUB_PORTRAIT),
};
...
static const dyesub_pagesize_t hiti_p520l_page[] =
{
  ...
  DEFINE_PAPER_SIMPLE( "w360h360", "5x5", PT(1540,300), PT(1548,300), DYESUB_LANDSCAPE),
  DEFINE_PAPER_SIMPLE( "w360h504", "5x7", PT(1548,300), PT(2140,300), DYESUB_PORTRAIT),
  DEFINE_PAPER( "w360h504-div2", "3.5x5*2", PT(1548,300), PT1(2140,300), 0, 0, 0, PT1(0,300), DYESUB_PORTRAIT),
  DEFINE_PAPER_SIMPLE( "w360h432", "6x5", PT(1588,300), PT(1844,300), DYESUB_LANDSCAPE),
  ...
  DEFINE_PAPER( "w432h576-div4", "2x6*4", PT(1844,300), PT2(2434,300), 0, 0, 0, PT2(0,300), DYESUB_PORTRAIT),
  DEFINE_PAPER_SIMPLE( "w432h648", "6x9", PT(1844,300), PT(2740,300), DYESUB_PORTRAIT),
};
...
static const dyesub_printsize_t hiti_p520l_printsize[] =
{
  ...
  // Also 5x3.74" 1548x1160 w360h270, code 0x08
  { "300x300", "w288h432", 1240, 1844},
  { "300x300", "w288h432-div2", 1248, 1844},
  { "300x300", "w360h360", 1540, 1548},
  { "300x300", "w360h504", 1548, 2140},
  { "300x300", "w360h504-div2", 1548, 2152},
  { "300x300", "w360h432", 1588, 1844},
  { "300x300", "w432h432", 1844, 1844},
  { "300x300", "w432h576", 1844, 2434},
  { "300x300", "w432h576-div2", 1844, 2492},
  { "300x300", "w432h576-div4", 1844, 2492},
  { "300x300", "w432h648", 1844, 2740},
};
...
static void hiti_printer_start(stp_vars_t *v, int model)
{
  ...
  if (!strcmp(pd->pagesize, "B7"))
	  ...
	  pgcode = 11;
  else if (!strcmp(pd->pagesize, "w360h432"))
	  pgcode = 20;
  else if (!strcmp(pd->pagesize, "w432h432"))
	  pgcode = 21;
  else if (!strcmp(pd->pagesize, "w432h576"))
...
static const dyesub_pagesize_t hiti_p510l_page[] =
{
  ...
  DEFINE_PAPER( "w288h432-div2", "2x6*2", PT1(1248,301), PT(1844,301), 0, PT1(0,300), 0, 0, DYESUB_LANDSCAPE),
  DEFINE_PAPER_SIMPLE( "w360h504", "5x7", PT(1548,301), PT(2140,301), DYESUB_PORTRAIT),
  DEFINE_PAPER( "w360h504-div2", "3.5x5*2", PT(1548,301), PT1(2140,301), 0, 0, 0, PT1(0,300), DYESUB_PORTRAIT),
  DEFINE_PAPER_SIMPLE( "w432h576", "6x8", PT(1844,301), PT(2434,301), DYESUB_PORTRAIT),
  DEFINE_PAPER( "w432h576-div2", "4x6*2", PT(1844,301), PT1(2434,301), 0, 0, 0, PT1(0,300), DYESUB_PORTRAIT),
...
static const dyesub_cap_t dyesub_model_capabilities[] =
{
  ...
  { /* Canon CP-10 */
    ...
    DYESUB_FEATURE_FULL_WIDTH | DYESUB_FEATURE_FULL_HEIGHT
      ...
      | DYESUB_FEATURE_PLANE_INTERLACE | DYESUB_FEATURE_HASBACKEND,
  ...
  { /* Canon CP820, CP910, CP1000, CP1200, CP1300, CP1500 */
    1011,
    &cmy_ink_list,
    &res_300dpi_list,
    &cp910_page_list,
  ...
  { /* Kodak Professional 1400 */
    ...
    DYESUB_FEATURE_FULL_WIDTH
      ...
      | DYESUB_FEATURE_ROW_INTERLACE | DYESUB_FEATURE_HASBACKEND,
